# Hi, I'm Ryan

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ryankim17920/)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:ryankim17920@gmail.com)

I'm a student researcher working on the applications of **AI in healthcare**. I build ML systems that solve real problems in oncology, genomics, and medical imaging.

## What I'm Working On

- **Research Intern @ [AI in Medicine & Computational Biology Lab, WCM](https://ihlab.med.cornell.edu/)** — Investigating systematic bias in AI tumor purity estimators across TCGA and internal cohorts.
- **[Simons Summer Research Fellow](https://www.stonybrook.edu/simons/) @ [Yurovsky Lab, Stony Brook](https://ayurovsky.github.io/)** — Developed DNABERT-based prognostic models predicting glioblastoma survival from non-coding regulatory DNA mutations.
- **Founder @ [Papers2Code.org](https://papers2code.org)** — Platform indexing 300k+ ML/AI papers lacking code, with a pipeline mapping papers to GitHub repos.

## Previous Research

- **Research Intern @ [Advanced Computing & Oncology Lab, MSK](https://www.mskcc.org/research-areas/research-teams/advanced-computing-and-oncology-lab)** — Built DSPy-optimized LLM pipelines to extract disease progression events from clinical reports. Abstract accepted at ACRO 2026.
